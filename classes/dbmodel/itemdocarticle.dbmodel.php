 ...
 * @Time: 下午4:22
 */
class ItemDocArticleDbModel extends DbModel{
    ...
    protected $_tableName = 'info_item_doc_article';
...
//            Mcache::set(self::MC_ARTICLE_INFO . $id, $data);
     ...
     * @return bool|multitype
     */
    public function getByItem($id, $item, $real=false){
//        $data = Mcache::get(self::MC_ARTICLE_INFO . $id);
//        if(!$data || $real){
        $data = $this->getByItemReal($id, $item);
//        }
        return $data;
    }

    public function getByItemReal($id, $item){
...
//            Mcache::set(self::MC_ARTICLE_HOT . $mid, $data);
     ...
     * @return Ambigous
     */
    public function clicks($articleId, $item){
        $where = "`id` = '".$articleId."' AND `item` = '".$item."'";
     ...
     * @return Ambigous
     */
    public function goodNum($articleId, $item){
        $where = "`id` = '".$articleId."' AND `item` = '".$item."'";
     ...
     * @return Ambigous
     */
    public function badNum($articleId, $item){
        $where = "`id` = '".$articleId."' AND `item` = '".$item."'";
...
//            Mcache::set($mcKey, $data);
     ...
     * 获取最旧的一条记录
     */
    public function getOneMostOld($item){
        $where = "`item` = '".$item."'";
        $order = '`id` ASC';
        $limit = '0, 1';
        $sql = 'SELECT * FROM '.$this->_tableName.' WHERE '.$where.' ORDER BY '.$order.' LIMIT '.$limit;

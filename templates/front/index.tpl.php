<?php
include 'header.tpl.php';
?>
<header class="jumbotron masthead">
  <div class="inner">
    <h1>LaneBlog</h1>
    <p>蝼蚁虽小，也有梦想</p>
  </div>
</header>
<div class="container-fluid">
    <div class="row-fluid">
        <div class="span8">
            <div class="row-fluid">
        		<div class="span12 page-header">
					<p class="index-title">深入理解PHP之数组(遍历顺序)</p>
        			<p>2014-01-12 00:22:15，Power By 李轩</p>
        			<p>Tag1，Tag2，Tag3</p>
        			<p>这个是我上周末在”阿里PHP技术沙龙”临时分享的一个主题的PPT, 主要是介绍一下Zend Optimizer Plus(简称O+).</p>
        			<p>O+是由Zend公司开发的一个PHP性能提升工具, 在PHP5.5开始, 已经随着PHP的源代码一起发布了, 并且也改名为:Opcache.</p>
        		</div>
        	</div>
        	<div class="row-fluid">
        		<div class="span12 page-header">
					<p class="index-title">深入理解PHP之数组(遍历顺序)</p>
        			<p>2014-01-12 00:22:15，Power By 李轩</p>
        			<p>Tag1，Tag2，Tag3</p>
        			<p>这个是我上周末在”阿里PHP技术沙龙”临时分享的一个主题的PPT, 主要是介绍一下Zend Optimizer Plus(简称O+).</p>
        			<p>O+是由Zend公司开发的一个PHP性能提升工具, 在PHP5.5开始, 已经随着PHP的源代码一起发布了, 并且也改名为:Opcache.</p>
        		</div>
        	</div>
        </div>
        <div class="span4">
        	<div class="row-fluid">
        		<div class="span12">
        			<h3>最热门的文章</h3>
        			<p>PHP文章1标题大概有20个字长度哦</p>
        			<p>文章2</p>
        			<p>文章3</p>
        			<p>文章4</p>
        			<p>文章5</p>
        		</div>
        	</div>
        	<div class="row-fluid">
        		<div class="span12">
					<h3>最新的评论</h3>
        			<p>文章1</p>
        			<p>文章2</p>
        			<p>文章3</p>
        			<p>文章4</p>
        			<p>文章5</p>
				</div>
        	</div>
        	<div class="row-fluid">
        		<div class="span12">
					<h3>作者简介</h3>
        			<p>作者简介，作者简介，作者简介</p>
				</div>
        	</div>
        </div>
    </div>
</div>
<div class="blank-line"></div>

<div class="container-fluid" >
    
</div>
<div class="container-fluid" >
	<div class="row-fluid div-margin">
        <div class="span12"	>
            <ul class="unstyled">
            	<li class="span1">友情链接</li>
            </ul>
        </div>
    </div>
    <div class="row-fluid" >
        <div class="span12"	>
            <ul class="unstyled">
            	<li class="span1"><a href="http://www.google.com">Google</a></li>
            	<li class="span1"><a href="http://www.PHP.net">PHP</a></li>
                <li class="span1"><a href="http://www.apache.net">Apache</a></li>
            	<li class="span1"><a href="http://www.mysql.net">Mysql</a></li>
            	<li class="span1"><a href="http://www.github.net">github</a></li>
            </ul>
        </div>
    </div>
</div>
<?php
include 'footer.tpl.php';
?>